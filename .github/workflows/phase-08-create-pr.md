...
pre-steps:
  ...
  - name: Verify and fetch previous phase state
    env:
      ...
      EXPECTED_PREVIOUS_WORKFLOWS: .github/workflows/phase-07-ui-validation.lock.yml
...
7. **Hand-off (only when `dry_run` is `false`):** call the exact `phase_09_pr_code_review`
    ...
    The PR number is not known yet at dispatch time
