---
# Source workflow for GitHub Agentic Workflows (gh-aw) — Phase 4 of 11 (Unit Test + Test Review).
# POC WORKFLOW: Runs in ai-shared, targets onrunning/on-frontend. Then run: gh aw compile
# Chain: ... 03 -> 04 -> 05 ... | on fail, dispatches back to phase-03-implement (fix loop).
on:
  workflow_dispatch:
    inputs:
      ticket:
...
# Validator: on pass advance to phase 5, on fail send work back to the Implementer (phase 3).
safe-outputs:
  upload-artifact:
    retention-days: 14
    max-uploads: 1
  dispatch-workflow: [phase-05-code-review, phase-03-implement]
...
# Phase 4 of 11 — Unit Test + Test Review (gh-aw dispatch chain)

You are the **Test Review** agent. You run in your own workflow with a fresh context. Add
or strengthen unit tests, run them, judge coverage against the acceptance criteria, then hand
off based on your verdict.
...
2. Do this phase's work and write `.pipeline/verdicts/04-test-review.json`. On `fail`, also
...
- On `status: "pass"`: emit `dispatch-workflow` for **`phase-05-code-review`**, passing
...
- On `status: "fail"`: if `loop_count` < 3, emit `dispatch-workflow` for
  **`phase-03-implement`**, passing `ticket`, `prev_run_id` = `${{ github.run_id }}`, and
  `loop_count` = `${{ inputs.loop_count }}` + 1. The Implementer will address the fix requests.
