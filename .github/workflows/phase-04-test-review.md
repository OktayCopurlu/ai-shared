...
pre-steps:
  ...
  - name: Verify and fetch previous phase state
    env:
      ...
      EXPECTED_PREVIOUS_WORKFLOWS: .github/workflows/phase-03-implement.lock.yml
...
- On `status: "pass"`: call the exact `phase_05_code_review` safe-output tool (never the generic
...
- On `status: "fail"`: if `loop_count` < 3, call the exact `phase_03_implement` safe-output tool
  ...
  The Implementer will address the fix requests.
