...
This skill does not explain every browser command. In gh-aw workflows, use `playwright-cli` from bash. It does not cover accessibility — load `a11y-audit` for that.
...
2. Navigate, interact, and snapshot — in gh-aw workflows, use `playwright-cli browser_navigate`, `browser_snapshot`, `browser_take_screenshot`, and `browser_run_code` from bash.
3. For deeper inspection, use console messages, network requests, snapshots, and page evaluation through `playwright-cli browser_run_code`.
...
4. For cookie or storage overrides, set the value directly on the current origin with `playwright-cli browser_run_code`, then reload. If storage helper tools are unavailable, use page evaluation with the confirmed cookie or storage key/value.
...
- `playwright-cli` — browser automation for navigation and interaction in gh-aw workflows
