...
- On `status: "pass"`: emit `dispatch-workflow` for **`phase-07-ui-validation`**, passing
  `ticket`, `prev_run_id` = `${{ github.run_id }}`, `loop_count` = `${{ inputs.loop_count }}`.
...
{{#runtime-import OktayCopurlu/ai-shared/experiment/github-delivery-pipeline/phases/06-qa.md@main}}
